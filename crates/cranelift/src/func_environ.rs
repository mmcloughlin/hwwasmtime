    ...
    fn translate_call(
        ...
        match name {
            ...
            Some("vsha1pq_u32") => {
                let v = builder
                    .ins()
                    .aarch64_sha1p(call_args[0], call_args[1], call_args[2]);
                let inst = builder.func.dfg.value_def(v).unwrap_inst();
                return Ok(inst);
            }
            Some("vsha1mq_u32") => {
                let v = builder
                    .ins()
                    .aarch64_sha1m(call_args[0], call_args[1], call_args[2]);
                let inst = builder.func.dfg.value_def(v).unwrap_inst();
                return Ok(inst);
            }
            Some("vsha1h_u32") => {
                let v = builder.ins().aarch64_sha1h(call_args[0]);
                let inst = builder.func.dfg.value_def(v).unwrap_inst();
                return Ok(inst);
            }
            Some("vsha1su0q_u32") => {
                let v = builder
                    .ins()
                    .aarch64_sha1su0(call_args[0], call_args[1], call_args[2]);
                let inst = builder.func.dfg.value_def(v).unwrap_inst();
                return Ok(inst);
            }
            _ => {}
